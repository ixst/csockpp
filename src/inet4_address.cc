...
Inet4Address::Inet4Address(struct sockaddr_in addr) noexcept
    : InetAddress(&addr, sizeof(addr)),
      addr_in_((struct sockaddr_in&)(*data_))
{}

Inet4Address::Inet4Address(std::string ip, uint16_t port)
...
void Inet4Address::port(uint16_t value) noexcept {
...
void Inet4Address::ip(std::string value) {
...
bool Inet4Address::operator==(const sockaddr_in& addr) const noexcept {
